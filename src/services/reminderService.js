...
const createReminder = async (courseId, userId, reminderSchedule) => {
			...
			try {
				...
				await Promise.race([
					...
					new Promise((_, reject) =>
						setTimeout(() => reject(new Error('Telegram send timeout')), 5000)
					),
				])
				console.log(
	...
	const supplementTimes = [
		{
			time: '08:00',
			supplements: reminderSchedule.morning || [],
			period: 'утренние',
		},
		{
			time: '14:00',
			supplements: reminderSchedule.afternoon || [],
			period: 'дневные',
		},
		{
			time: '20:00',
			supplements: reminderSchedule.evening || [],
			period: 'вечерние',
		},
	]
